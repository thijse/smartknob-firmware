...
void AngleSelectorApp::initScreen()
{
    ...
    int line1_y_offset = 60;
    int line2_y_offset = 84;

    // Create semi transparant grey overlay behind text for readability. A square with rounded corners
    {
        lv_obj_t *rect = lv_obj_create(screen);
        lv_obj_set_size(rect, 130, 60);
        lv_obj_align(rect, LV_ALIGN_CENTER, 0, (line1_y_offset + line2_y_offset) / 2);
        lv_obj_set_style_radius(rect, 8, 0);
        lv_obj_set_style_bg_color(rect, LV_COLOR_MAKE(0x28, 0x28, 0x28), 0);
        lv_obj_set_style_bg_opa(rect, LV_OPA_70, 0);
        lv_obj_clear_flag(rect, LV_OBJ_FLAG_CLICKABLE); // Make the rectangle non-interactive
        lv_obj_set_style_border_width(rect, 0, 0);      // remove border
    }
    ...
    // Outline labels to simulate text stroke (created before white labels to render underneath)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            lv_obj_align(label_primary_outline_[i], LV_ALIGN_CENTER, dx[i], line1_y_offset + dy[i]);
            ...
            lv_obj_align(label_secondary_outline_[i], LV_ALIGN_CENTER, dx[i], line2_y_offset + dy[i]);
        }
    }
    ...
    lv_obj_align(label_primary_, LV_ALIGN_CENTER, 0, line1_y_offset);
    ...
    lv_obj_align(label_secondary_, LV_ALIGN_CENTER, 0, line2_y_offset);
}
