...
typedef struct _PB_AppSelect {
    pb_size_t which_selector;
    union {
        /* Select by numeric app index (0..255). Preferred for compactness. */
        uint8_t by_id;
        /* Alternative: select by string app_id (e.g., "light_dimmer"). */
        char by_app_id[33];
    } selector;
} PB_AppSelect;
...
#define PB_AppSelect_FIELDLIST(X, a) \
X(a, STATIC,   ONEOF,    UINT32,   (selector,by_id,selector.by_id),   1) \
