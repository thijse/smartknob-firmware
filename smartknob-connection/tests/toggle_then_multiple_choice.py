...
# Enable INFO logging to see debug messages
...
async def main():
    # Ask user if they want to reset between switches
    print("🔄 Reset Configuration")
    print("Do you want to reset the device between component switches?")
    print("  y/Y - Yes, reset between switches (more stable)")
    print("  n/N - No, switch without reset (faster)")
    
    while True:
        try:
            user_input = input("Choice (y/n): ").strip().lower()
            if user_input in ['y', 'yes']:
                reset_between_switches = True
                print("✅ Will reset device between switches")
                break
            elif user_input in ['n', 'no']:
                reset_between_switches = False
                print("✅ Will switch without reset")
                break
            else:
    ...
    try:
        async with SmartKnobConnection(port) as connection:
            ...
            async with anyio.create_task_group() as tg:
                tg.start_soon(connection.start_read_loop)  # fast producer
                ...
                # Step 1: Send Toggle
                toggle_id = "clean_toggle"
                prev_active_count = monitor.component_mode_active_count
                try:
                    # Use explicit symmetric feel for determinism (snap_point=0.5, snap_point_bias=0.0)
                    # To try asymmetric feel, pass e.g.: snap_point=0.7, snap_point_bias=0.4
                    nonce_t = await connection.send_toggle(
                ...
                # Allow a short dwell to interact
                print("⏳ Pause on Toggle for 2s; rotate/press if desired...")
                await anyio.sleep(2.0)

                # Transport handshake before MultipleChoice to confirm link health
                print("🚦 Handshake: GET_KNOB_INFO before MultipleChoice")
                try:
                    nonce_h = await connection.send_command(0)  # GET_KNOB_INFO
                    tsx = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    monitor._log_to_file(f"[{tsx}] ENQUEUED Command nonce={nonce_h} GET_KNOB_INFO(0)")
                    ack_ok_h = await monitor.wait_for_ack(nonce_h, timeout=1.5)
                    if not ack_ok_h:
                        print(f"⚠️ Warning: ACK timeout for GET_KNOB_INFO nonce={nonce_h}")
                        monitor._log_to_file(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] ⚠️ ACK timeout for GET_KNOB_INFO nonce={nonce_h}")
                except Exception as e:
                    print(f"❌ Failed to send GET_KNOB_INFO: {e}")

                # Small stabilization before sending MultipleChoice
                await anyio.sleep(0.3)

                # Step 2: Send MultipleChoice
                mc_id = "drink_selector"
                drinks = ["Coffee", "Tea", "Water", "Juice", "Soda"]
                prev_active_count = monitor.component_mode_active_count
                try:
                    nonce_m = await connection.send_multi_choice(
                        component_id=mc_id,
                    ...
                    tsx = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    monitor._log_to_file(f"[{tsx}] ENQUEUED AppComponent nonce={nonce_m} id='{mc_id}' type=MULTI_CHOICE options={len(drinks)}")

                    ack_ok = await monitor.wait_for_ack(nonce_m, timeout=1.5)
                    if not ack_ok:
                        print(f"⚠️ Warning: ACK timeout for MultipleChoice AppComponent nonce={nonce_m}")
                        monitor._log_to_file(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] ⚠️ ACK timeout for MultipleChoice nonce={nonce_m}")

                    rx_ok = await monitor.wait_for_rx_app_component(mc_id, timeout=3.0)
                    if not rx_ok:
                        print("⚠️ Warning: No firmware RX AppComponent log observed for MultipleChoice")

                    act_ok = await monitor.wait_for_component_mode_active_increment(prev_active_count, timeout=5.0)
                    if act_ok:
                        print("✅ MultipleChoice component activated")
                    else:
                        print("⚠️ Warning: Did not observe 'Component mode active' increment after MultipleChoice")
                ...
                try:
                    while True:
                        ...
                        if reset_between_switches:
                            print("🔄 Resetting device before Toggle...")
                            try:
                                # Use integrated reset method (handles connection state automatically)
                                ...
                                if reset_success:
                                    ...
                                    print("⚠️ Warning: Device reset failed, continuing anyway...")
                                    
                            except Exception as e:
                                print(f"⚠️ Reset error: {e}, continuing without reset...")
                        ...
                        # Reset device if requested before MultipleChoice
                        if reset_between_switches:
                            print("🔄 Resetting device before MultipleChoice...")
                            try:
                                # Use integrated reset method (handles connection state automatically)
                                reset_success = await connection.reset_device()
                                if reset_success:
                        ...
                        # Step C: Send MultipleChoice again
                        mc_id = "drink_selector"
                        drinks = ["Coffee", "Tea", "Water", "Juice", "Soda"]
                        prev_active_count = monitor.component_mode_active_count
                        try:
